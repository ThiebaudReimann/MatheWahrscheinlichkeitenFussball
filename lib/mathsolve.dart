...
void runSequence(Options options) {
  ...
  for (int i = 0; i < groups.length; i++) {
    ...
    for (int j = 0; j < gruppe.length; j++) {
      for (int k = j + 1; k < gruppe.length; k++) {
        ...
        // Statistik anpassen
        statistik[gruppe[j]]!['toreErzielt'] = statistik[gruppe[j]]!['toreErzielt']! + tore1;
        statistik[gruppe[j]]!['toreErhalten'] = statistik[gruppe[j]]!['toreErhalten']! + tore2;
        statistik[gruppe[k]]!['toreErzielt'] = statistik[gruppe[k]]!['toreErzielt']! + tore2;
        statistik[gruppe[k]]!['toreErhalten'] = statistik[gruppe[k]]!['toreErhalten']! + tore1;
        
        if (tore1 > tore2) {
          statistik[gruppe[j]]!['punkte'] = statistik[gruppe[j]]!['punkte']! + 3;
        } else if (tore2 > tore1) {
            statistik[gruppe[k]]!['punkte'] = statistik[gruppe[k]]!['punkte']! + 3;
        } else {
          statistik[gruppe[j]]!['punkte'] = statistik[gruppe[j]]!['punkte']! + 1;
          statistik[gruppe[k]]!['punkte'] = statistik[gruppe[k]]!['punkte']! + 1;
        }
      ...
      statistik.forEach((team, stats) => print("${team.name} hat ${stats['punkte']} punkte, toreErzielt: ${stats['toreErzielt']}, toreErhalten ${stats['toreErhalten']}"));

    // Gruppensieger und zweitplazierte bestimmen
      final List<Manschaft> qualifizierte = calculator.getQualifikationsTeams(statistik);
      gruppenGewinner.addAll(qualifizierte); // Füge die beiden weiterkommenden Teams zur KO-Runde hinzu
         print("${greenText}Die Gewinner sind ${qualifizierte[0].name} und ${qualifizierte[1].name} aus Gruppe ${String.fromCharCode(65 + i)}!${reset}");
  }
  ...
  final List<String> stageStrings = calculator.berechneStages(koTeilnehmer.length);
   ...
   for (int i = 0; i < stageStrings.length; i++) {
    ...
    for (int j = 0; j < koTeilnehmer.length; j += 2) {
      final Manschaft gewinner = calculator.calculateWinner(koTeilnehmer[j], koTeilnehmer[j + 1]);
      runde.add(gewinner);
        print("${(j / 2 + 1).toInt()}: ${italic + underlined}${koTeilnehmer[j].name}${reset} vs. ${italic + underlined}${koTeilnehmer[j + 1].name}${reset} => ${bold + underlined}${gewinner.name}${reset}");
    }
